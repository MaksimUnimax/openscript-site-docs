...
All current frontend/design candidates live as local docs repo copies. Most now have matching exact-copy active app skills; `vercel_web_design_guidelines` remains docs-reference-only until a local-safe package exists.
...
| Candidate | Local server folder | Local files | full_skill_imported: yes/no | has_local_SKILL_md: yes/no | active_app_copy: yes/no | docs_reference_only: yes | can_be_used_as_basis_for_app_skill: yes/no/maybe | reason | recommended_next_action |
|---|---|---|---|---|---|---|---|---|---|
| manalkaff/opendesign | `/opt/openscript-site-docs/docs/codex_source/tools/frontend_design_skills/manalkaff_opendesign/` | `README.md`, `SKILL.md`, `SUMMARY.md`, `LOCAL_STATUS.md`, `imported_index.md` | yes | yes | yes | yes | yes | complete local skill copied from the public project; matching exact-copy active app skill exists | refresh from the local docs copy if the active app copy ever needs updating |
...
| vercel web-design-guidelines | `/opt/openscript-site-docs/docs/codex_source/tools/frontend_design_skills/vercel_web_design_guidelines/` | `README.md`, `SKILL.md`, `SUMMARY.md`, `LOCAL_STATUS.md`, `imported_index.md` | yes | yes | no | yes | maybe | complete local skill copied from the public project, but the clean source requires external WebFetch and is docs-reference-only for now | keep as docs-reference-only until a local-safe package is defined |
...
| mblode/agent-skills | `/opt/openscript-site-docs/docs/codex_source/tools/frontend_design_skills/mblode_agent_skills/` | `README.md`, `SKILL.md`, `product-ui.md`, `marketing-ui.md`, `aesthetic-direction.md`, `design-in-code.md`, `references/cro.md`, `references/testing.md`, `references/modern.md`, `SUMMARY.md`, `LOCAL_STATUS.md`, `imported_index.md` | yes | yes | yes | yes | yes | complete local skill copied from the public project; companion files are imported locally and the matching exact-copy active app skill exists | refresh from the local docs copy if the active app copy ever needs updating |
...
The clean active app skills already exist as exact local copies under `/opt/ai-starter-community/.agents/skills/**`, except `vercel_web_design_guidelines`, which stays docs-reference-only until a local-safe package exists.
To refresh any active app copy, use the matching docs-repo local copy as input.
