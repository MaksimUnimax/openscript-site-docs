# Hermes API server

STATUS: verified
CATEGORY: vendor/hermes/runtime

SOURCE_URLS:
- https://hermes-agent.nousresearch.com/docs/user-guide/features/api-server/

SOURCE_PATHS:
- docs/codex_source/vendor/hermes/_raw/api_server.html
- docs/codex_source/vendor/hermes/_raw/llms-full.txt

LOCAL_SOURCE_PATHS:
- /opt/openscript-agent-lab/vendor/hermes-agent/gateway/config.py
- /opt/openscript-agent-lab/vendor/hermes-agent/gateway/platforms/api_server.py

CLI_HELP:
- docs/codex_source/vendor/hermes/_raw/cli_help/hermes_gateway_help.txt

APPLIES_TO:
- API server runtime
- API_SERVER_ENABLED
- API_SERVER_KEY
- local endpoint defaults

FACTS:
- the API server is an explicit runtime mode.
- localhost is the default bind target.
- `API_SERVER_KEY` enables bearer auth.
- browser CORS is off by default and must be enabled explicitly.

CONTRACT:
- the API server is not the same thing as provider auth.
- keep non-loopback exposure behind deliberate security controls.

NON_GOALS:
- no frontend implementation details

FORBIDDEN_MISUSE:
- exposing the API server without auth or origin controls

OPEN_QUESTIONS:
- exact deployment topology depends on the consuming frontend

VERIFICATION:
- raw_source_saved: yes
- official_docs_checked: yes
- local_source_checked: yes
- cli_help_checked: yes
- verified_at_utc: 2026-05-16T10:33:42Z
- extraction_run_id: OPENSCRIPT_AGENT_LAB_CODEX_SOURCE_HERMES_DOCS_EXTRACTION_20260516_01
