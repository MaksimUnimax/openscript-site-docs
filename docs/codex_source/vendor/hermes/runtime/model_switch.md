...
APPLIES_TO:
- `/model` switching
- provider/api_mode recomputation

FACTS:
- model switching delegates back to the same runtime provider machinery.
- the helper recomputes api_mode when the active provider changes.

CONTRACT:
- model switching logic must remain centralized with runtime provider helpers.

NON_GOALS:
- no UI behavior spec

FORBIDDEN_MISUSE:
- duplicating model/provider selection in command handlers
